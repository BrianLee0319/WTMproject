...
class AccumulatorExampleModuleImp(outer: AccumulatorExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val doMul = funct === UInt(3)
  val memRespTag = io.mem.resp.bits.tag(log2Up(outer.n)-1,0)

  // datapath
  val rdata = cmd.bits.rs1
  //multiplier
  val Mul = Module(new GCD)
  Mul.io.data0 := regfile(0)
  Mul.io.data1 := regfile(1)
  Mul.io.e := doMul

  when (doMul) {
    regfile(addr+1.U) := Mul.io.sum0
    regfile(addr) := Mul.io.sum1
  }
